...
// Fetch Employees
$employees = $con->query("SELECT DISTINCT full_name FROM users");

function convertTo12HourFormat($time) {
    return date('h:i:s A', strtotime($time)); // Convert to 12-hour format with AM/PM
}

function convertToFullDate($date) {
    return date('F j, Y', strtotime($date)); // e.g., March 19, 2025
}
...
<div class="container">
    ...
    <div class="employee-list">
        ...
        <?php while ($row = $employees->fetch_assoc()): ?>
            <a href="?employee=<?php echo $row['full_name']; ?>">
    ...
    <?php if (isset($_GET['employee'])): ?>
        ...
        $result = $con->query("SELECT * FROM users WHERE full_name = '$employee_name'");
        ?>
